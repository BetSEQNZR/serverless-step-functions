...
function getS3ListObjectsV2Permissions(state) {
  const bucket = state.Parameters['Bucket.$'] ? state.Parameters['Bucket.$'] : state.Parameters.Bucket;
  const prefix = state.Parameters['Prefix.$'] ? state.Parameters['Prefix.$'] : state.Parameters.Prefix;
  ...
  return _.flatMap(taskStates, (state) => {
    ...
    switch (resourceName) {
      ...
      case 'arn:aws:states:::s3:listObjectsV2':
        return getS3ListObjectsV2Permissions(state);
